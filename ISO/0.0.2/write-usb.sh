...
default_iso="$version_root/out/unexus-os-0.0.2-x86_64.iso"

usage() {
    cat <<EOF
Usage: sudo sh ISO/0.0.2/write-usb.sh /dev/sdX [path/to.iso]
...
Examples:
  ...
  sudo sh ISO/0.0.2/write-usb.sh /dev/sdb
  sudo sh ISO/0.0.2/write-usb.sh /dev/sdb ISO/0.0.2/out/unexus-os-0.0.2-x86_64.iso
EOF
}
...
if [ "$(id -u)" -ne 0 ]; then
    printf '[uNexus USB] run as root: sudo sh ISO/0.0.2/write-usb.sh %s\n' "$target" >&2
    exit 1
fi

if [ ! -f "$iso" ]; then
    printf '[uNexus USB] ISO not found: %s\n' "$iso" >&2
    printf '[uNexus USB] build it first: sudo sh ISO/0.0.2/build-iso.sh\n' >&2
    exit 1
fi
